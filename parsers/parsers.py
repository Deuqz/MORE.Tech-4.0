...
class LentaRuParser(Parser):
    ...
    def get_news_timed(self, get_text=False, delta_time=datetime.timedelta(days=30)):
        news = []
        for delta in range(delta_time.days // 15 + 1):
        ...
        try:
            ...
            for item in items:
                ...
                if kwargs.get('get_text'):
                    ...
                    news.append(News(tag="business", site='ria', header=header.string, date=date, views=views,
...
class KlerkRuParser(Parser):
    ...
    SITE='https://www.klerk.ru/buh/news/'
    ...
    def get_news_timed(self, get_text=False, delta_time=datetime.timedelta(days=30)):
        ...
        for delta in range(1, 100):
            with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
                futures = [executor.submit(self.get_page_news, delta * 15 + i) for i in range(15)]
                for future in concurrent.futures.as_completed(futures):
                    nw = future.result()
                    news += nw
    ...
    def get_page_news(self, page_num, get_text=False):
        soup = BeautifulSoup(requests.get(self.SITE + f'page/{page_num}').text,
                             'html.parser')
        ...
        for article in articles:
            ...
            date = parse(article.select_one(r'[class="feed-item__stats"]').span.find('core-date-format')['date'])#.select_one(r'[class="core-date-format"]')
